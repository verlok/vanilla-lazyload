...
| `rootMargin`        | A string with syntax similar to the CSS margin property. This allows you to adjust the bounds of the scrolling area outward, so that the elements to be loaded are shown before they phisically enter the scrolling area. **Overrides `threshold`**. Available **only in v. 10.x**. [More info](https://developer.mozilla.org/en-US/docs/Web/API/IntersectionObserver/rootMargin). | `null`        |
